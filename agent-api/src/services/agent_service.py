...
def _normalize_retrieved_contexts(raw_contexts: list[dict[str, Any]]) -> list[dict[str, Any]]:
...
def _extract_retrieved_contexts(messages: list[BaseMessage]) -> list[dict[str, Any]]:
    contexts: list[dict[str, Any]] = []
    for message in messages:
        if getattr(message, "name", None) != "retrieve_context":
            continue
    ...
    return _normalize_retrieved_contexts(contexts)
...
class AgentService:
    ...
    async def chat(self, question: str, user: str, session_id: Optional[str]) -> dict:
        ...
        retrieved_contexts = result.get("retrieved_contexts") or None
    ...
    async def _run_agent(
        ...
        retrieved_contexts = _extract_retrieved_contexts(result["messages"])
